...
"""
P_vec_size = int((num_node * num_node - num_node) / 2)  + num_node
P_vec = Parameter(torch.FloatTensor(torch.ones(P_vec_size)))
P_hat_symm = create_symm_matrix_from_vec(P_vec, num_node)
A_tilde = torch.FloatTensor(num_node, num_node)
A_tilde = torch.sigmoid(P_hat_symm) * S_adj1 + torch.eye(num_node)
