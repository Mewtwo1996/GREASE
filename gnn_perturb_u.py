...
import utils1
from utils1 import get_degree_matrix, normalize_adj, create_symm_Umatrix_from_vec, create_vec_from_symm_matrix
...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, dims, num_nodes, item_idx, user_id,dropout=0.3, lr=0.001, weight_decay=5e-4,
    ...
    def reset_parameters(self, eps=10 ** -4):
        self.P_vec_size = int(len(self.item_idx) * 2)
        self.P_vec = Parameter(torch.FloatTensor(torch.ones(self.P_vec_size)))
    def forward(self, x, adj):
        self.sub_adj = adj
        self.P_hat_symm = create_symm_Umatrix_from_vec(self.P_vec, self.item_idx,self.user_id, self.num_nodes)  # Ensure symmetry
        #print("P is set")
